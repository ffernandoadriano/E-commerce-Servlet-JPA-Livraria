...
public class SalvarLivroAction extends Action {
	...
	public void process() throws Exception {
		...
		// Se existem erros, recoloca os dados na request e retorna para a própria
		// página
		if (existemErros()) {
	...
	/**
	 * Recoloca os dados preenchidos na request, para permitir que o formulário já
	...
	/**
	 * Adiciona um erro de validação. Os erros são armazenados em uma lista,
	 * inserida na request
	 * 
	 * @param erro
	 */
	...
	private void validarAno(String ano) {
		...
		} else if (!StringUtil.isInteger(ano)) {
	...
	private void validarPreco(String preco) {
		if (StringUtil.isEmpty(preco)) {
			adicionarErro("O preço é obrigatório");

		} else if (!StringUtil.isDouble(preco)) {
